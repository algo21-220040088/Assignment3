...
def option_volatility_arbitrage_strategy(wind_id, start_dt, end_dt):
    volatility = calc_v2_v1(wind_id, start_dt, end_dt)

    volatility.to_csv('E:/XX/option_arbitrage/data/mid/csv/volatility_04011436.csv')
    ...
    total_trade_dt = volatility.shape[0]
    ...
    for i in range(total_trade_dt):
        ...
        if state == 0:
            if volatility.loc[i, 'v2-v1-mean'] > 2*volatility.loc[i, 'std'] \
                    and volatility.loc[i, 'end_date_1'] >= volatility.loc[i+1, 'trade_date_int']:
                print('# 开仓,卖出远月，买入近月')
                # 开仓,卖出远月，买入近月
            ...
            if volatility.loc[i, 'v2-v1-mean'] < 0 \
                    or long_position.iloc[-1]['end_date'] < volatility.loc[i+1, 'trade_date_int']:
                print('# 平仓：卖出近月，买入远月')
                # 连接.db文件
                ...
                c = my_db.cursor()
                # 日期 和 wind_id
                x1 = volatility.loc[i, 'trade_date_int']
                y1 = long_position.iloc[-1]['wind_id']
                # x1 = int(x1.strftime('%Y%m%d'))
                # 根据wind_id找出期权的当日价格
                select_part = 'SELECT SETTLE'
                ...
                # 日期 和 wind_id
                x2 = volatility.loc[i, 'trade_date_int']
            ...
            if volatility.loc[i, 'v2-v1-mean'] > 0 \
                    or short_position.iloc[-1]['end_date'] < volatility.loc[i+1, 'trade_date_int']:
                print('# 平仓：卖出远月，买入近月')
                # 连接.db文件
                my_db = sqlite3.connect('E:\\XX\\option_arbitrage\\data\\raw\\option.db')
                # 对数据库进行操作，取出所需数据
    ...
    plt.legend(['gross_value'], loc='upper left')
    plt.show()
    ...
    position.to_csv('E:\\XX\\option_arbitrage\\data\\mid\\csv\\position.csv')


if __name__ == "__main__":
    option_volatility_arbitrage_strategy('510050.SH', 20170701, 20200701)